    ...
    <>
      ...
      <div className="box p-5 mt-5">
        <Table>
          ...
          <Table.Tbody>{rows.map((row) => <Table.Tr key={row.id}><Table.RowHeader className="font-medium">{row.label}</Table.RowHeader><Table.Td>{row.year}</Table.Td><Table.Td>{row.month}</Table.Td><Table.Td>Q{row.quarter}</Table.Td><Table.Td>{formatDisplayDate(row.start_date)}</Table.Td><Table.Td>{formatDisplayDate(row.end_date)}</Table.Td><Table.Td><span className={row.status === "closed" ? "text-danger" : "text-success"}>{row.status}</span></Table.Td><Table.Td className="text-right"><div className="flex justify-end gap-2"><Button size="sm" variant="outline-secondary" aria-label={`Edit reporting period ${row.label}`} title="Edit reporting period" onClick={() => openEdit(row)}><Lucide icon="FilePenLine" className="w-4 h-4" /></Button><Button size="sm" variant="outline-primary" onClick={() => void toggleStatus(row)}>{row.status === "closed" ? "Reopen" : "Close"}</Button></div></Table.Td></Table.Tr>)}</Table.Tbody>
        </Table>
      </div>
