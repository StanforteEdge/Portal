  ...
  return (
    <>
      ...
      <div className="grid grid-cols-12 gap-6 mt-5">
        <div className="col-span-12 xl:col-span-6 box p-5"><div className="font-medium mb-4">Customers</div><Table><Table.Thead><Table.Tr><Table.Th>Name</Table.Th><Table.Th>Email</Table.Th><Table.Th>Phone</Table.Th><Table.Th className="text-right">Action</Table.Th></Table.Tr></Table.Thead><Table.Tbody>{customers.map((row) => <Table.Tr key={row.id}><Table.Td>{row.name}</Table.Td><Table.Td>{row.email || "-"}</Table.Td><Table.Td>{row.phone || "-"}</Table.Td><Table.Td className="text-right"><Button size="sm" variant="outline-secondary" onClick={() => { setCustomerId(row.id); setCustomerForm({ name: row.name || "", email: row.email || "", phone: row.phone || "", address: row.address || "", tax_number: row.tax_number || "" }); setShowCustomer(true); }}><Lucide icon="FilePenLine" className="w-4 h-4" /></Button></Table.Td></Table.Tr>)}</Table.Tbody></Table></div>
        <div className="col-span-12 xl:col-span-6 box p-5"><div className="font-medium mb-4">Vendors</div><Table><Table.Thead><Table.Tr><Table.Th>Name</Table.Th><Table.Th>Email</Table.Th><Table.Th>Phone</Table.Th><Table.Th className="text-right">Action</Table.Th></Table.Tr></Table.Thead><Table.Tbody>{vendors.map((row) => <Table.Tr key={row.id}><Table.Td>{row.name}</Table.Td><Table.Td>{row.email || "-"}</Table.Td><Table.Td>{row.phone || "-"}</Table.Td><Table.Td className="text-right"><Button size="sm" variant="outline-secondary" onClick={() => { setVendorId(row.id); setVendorForm({ name: row.name || "", email: row.email || "", phone: row.phone || "", address: row.address || "", tax_number: row.tax_number || "" }); setShowVendor(true); }}><Lucide icon="FilePenLine" className="w-4 h-4" /></Button></Table.Td></Table.Tr>)}</Table.Tbody></Table></div>
      </div>
