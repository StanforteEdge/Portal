<?php /* Template Name:  Staff: Requests - Edit  */
?>
...
if ($id) {
    $requested = $wpdb->get_row("SELECT a._ID, a.staff, a.description,a.reimburse, a.type, a.amount, a.cct_created, a.due_date, a.status, a.team, a.project, b.title, c.first_name, c.last_name, c.email, d.name, e.name as request_type, e.code, e.approval
    ...
    $items = $wpdb->get_results(" SELECT * FROM staff_jet_cct_financial_items a
     WHERE a.request = $id AND a.type = 'request' ");

     print_r($items);
    $files = $wpdb->get_results("SELECT a.url, b._ID, b.request, a.type FROM staff_jet_cct_files a LEFT JOIN staff_jet_cct_financial_items b ON b._ID = a.row_id WHERE b.request = $id ");
                ...
                <div class=" col-span-12 lg:col-span-7">
                    ...
                    <div class="gap-3 sm:flex flex-row items-end">
                        <div class="mt-5 w-1/2">
                            ...
                            <select id="type" data-placeholder="Select Request Type" aria-label="Select Request Type" class="tom-select w-full" name="type" required>
                                <option value="<?= $requested->type; ?>" selected><?= $requested->request_type; ?></option>
                                <?php foreach ($requestTypes as $requestType) :?>
                                        <option value="<?= $requestType->_ID; ?>" data-name="<?= $requestType->name; ?> " aria-label="<?= $requestType->name; ?>"><?= $requestType->name; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-check  gap-2 flex-col items-start form-switch">
                            ...
                            <input id="reimburse" name="reimburse" class="form-check-input" aria-label="Check"  type="checkbox" <?= $requested->reimburse === '1' ? 'checked' : '';?> >
                        </div>
                    ...
                    <div class="gap-3 sm:flex flex-row">
                        <div class="mt-5 w-full">
                            ...
                            <select id="team" class="tom-select w-full" name="team" aria-label="Select Team">
                                <option value="<?= $requested->team; ?>" selected aria-label="<?= $requested->name; ?>"><?= $requested->name; ?></option>
                        ...
                        <div class="w-full mt-5">
                            ...
                            <select id="project" class="tom-select w-full" name="project" aria-label="Select Project">
                                <option value="<?= $requested->project; ?>"  selected aria-label="<?= $requested->title; ?>"><?= $requested->title; ?></option>
                    ...
                    <div class="gap-3 sm:flex flex-row">
                        <div class="w-full mt-5">
                            ...
                            <select id="category" data-placeholder="Select Category" class="tom-select w-full" name="category" required>
                        ...
                        <div class="w-full mt-5">
                            ...
                            <input type="text" class="form-control datepicker" value="<?=$requested->due_date;?>" name="date" id="date" data-single-mode="true" />
                        </div>
                    </div>
                    <div class="w-full mt-5">
                        <label for="purpose" class="form-label  ">Purpose</label>
                        <textarea id="purpose" name="purpose" class="form-control" placeholder="Purpose" required><?= $requested->description;?></textarea>
                    </div>
                    <div class="w-full hidden mt-5">
                        <label for="note" class="form-label  ">Note</label>
                        <textarea id="note" name="note" class="form-control" placeholder="Please add extra details. E.g Vendor account details"><?= $requested->description;?></textarea>
                    </div>
                </div>
            ...
            <div class="mt-4 pb-2 border-b">
                <div id="items-container" class="mt-4">
                    ...
                    ?>
                        ...
                        <div class="itemm border-t border-b p-3 flex flex-col gap-5 mt-2" data-item-index="<?= $index;?>">
                    <div class="flex gap-5">
                        <div class="w-full">
                            <label class="form-label">Item:</label>
                            <input type="text" name="item[<?= $index;?>][item]" required class="form-control" value="<?= $item->item;?>">
                        </div>
                        <div class="">
                            <label class="form-label">Quantity:</label>
                            <input type="number" name="item[<?= $index;?>][qty]" required class="form-control" value="<?= $item->quantity;?>">
                        </div>
                        <div class="">
                            <label class="form-label">Price:</label>
                            <input type="number" val="0" name="item[<?= $index;?>][price]" required class="form-control" value="<?= $item->price;?>">
                        </div>
                        <div class="">
                            <label class="form-label">Amount:</label>
                            <input type="number" name="item[<?= $index;?>][amt]" disabled class="form-control" value="<?= $item->quantity * $item->price;?>">
                        </div>
                    </div>
                    <div class="flex gap-5 items-end p-2 justify-between">
                        <div class="flex items-start gap-5">
                            <div class="">
                                ...
                                <textarea name="item[<?= $index;?>][account]" rows="1" required class="form-control" placeholder="Account Number | Account Name | Bank"><?= $item->account;?></textarea>
                            </div>
                            <div class="">
                                <label class="form-label"> Supporting Document(s):</label>
                                <input type="file" name="item[<?= $index;?>][file]"  class="form-control p-1 border" multiple>
                            </div>
                                <div class="form-check  gap-2 flex-col form-switch">
                                    <label class="form-check-label" for="item[<?= $index;?>][cash]">Cash Advance</label>
                                    <input id="item[<?= $index;?>][cash]" name="item[<?= $index;?>][cash]" class="form-check-input" type="checkbox" <?= $item->cash === 1 ? 'checked' : '';?>>
                                </div>
                        </div>
...
<script>
    $(document).ready(function($) {
        ...
        function submitForm(statusValue) {
            ...
            if ($('#type').val() === "" || $('#type').val() === "" || $('#project').val() === "" || $('#date').val() === "" || $('#purpose').val() === "") {
