<?php /* Template Name: Staff: Requests - Approval View */
?>

<?php
get_header();
$b_link = '/requests/approvals';
$b_title = 'Approvals';
$p_title = 'Request';
...
if (in_array('coo', (array) $user->roles)) {
...
} elseif (in_array('ed', (array) $user->roles)) {
    ...
    if ($id) {
        ...
        $items = $wpdb->get_results("SELECT * FROM staff_jet_cct_financial_items a WHERE a.request = $id AND a.type = 'request' ");
        $retired = $wpdb->get_results("SELECT * FROM staff_jet_cct_financial_items a WHERE a.request = $id AND a.type = 'retired");
    ...
    // Redirect to /requests page
    echo "<script>alert('Page only visible to Staff with COO/ED permissions.')</script>";
...
if (in_array($requested->status, [7, 9])) {
    ...
    $btnText = 'Unapproved';
    $message = "Request is unapproved.";
    $btnHide = "";
    $btnApprove = 'Approve';
    $app_message = 'Are you sure you want to approve this request?';
    $subject = "Re:$requested->request_type - $id - Approved";
    $content = "<p>$requested->request_type - $requested->code$id has been approved. </p>
        <p>Please login to your dashboard to disburse</p>
        <p><a class='btn' href='https://staff.stanforteedge.com/accounts/requests/request/?id=$id'/>Disburse</a></p>";
    $to = 'accounts@stanforteedge.com';
} elseif ($requested->status == 6) {
    $btnClass = 'btn-pending';
    $btnText = 'Pending';
    $message = 'Request has been cleared and is awating your approval.';
    $btnHide = "";
    $btnApprove = 'Approve';
    $app_message = 'Are you sure you want to approve this request?';
    ...
    if (in_array('ed', (array) $user->roles)) {
        ...
        if ($requested->approval == 2) {
            ...
            $btnText = 'Unapproved';
            $message = 'Request is awating Approval from the ED.';
            $btnHide = "hidden";
        } else {
            $btnClass = 'btn-outline-pending';
            $btnText = 'Approved';
            $message = 'Request is awating disbursement.';
            $btnHide = "hidden";
        }
    }
...
<div class="box p-5 mt-8">
    ...
    <?php if ($retired) : ?>
        <div class="overflow-x-auto border-t mt-5">
            ...
            <table class="table table-striped">
                ...
                <tbody>
                    ...
                    foreach ($retired as $retire) :
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="flex gap-2 flex-wrap">
                                    <?php foreach ($files as $file) :
                                        if ($file->request == $id || $file->type == 'retire') : ?>
                                            <a href="<?= $file->url; ?>" target="_blank"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-file-down w-4">
                                                    <path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z" />
                                                    <path d="M14 2v4a2 2 0 0 0 2 2h4" />
                                                    <path d="M12 18v-6" />
                    ...
                    <tr>
                        ...
                        <td><span class=" font-medium  text-lg ">&#x20A6;<?= $t_amount; ?></span></td>
                    </tr>
                    <tr>
                        <td colspan="3"></td>
                        <td class="font-medium text-lg">Diff:</td>
                        <td><span class=" font-medium  text-lg ">&#x20A6; <?= ($r_amount - $t_amount); ?></span></td>
                    </tr>
                </tbody>
            </table>
    ...
    $(document).ready(function($) {
        ...
        $('button[name="disapprove"], button[name="approve"]').click(function() {
            ...
            if (buttonName === 'disapprove') {
                ...
                statusValue = (<?= $requested->approval; ?> === 2 ? currentStatus + 2 : currentStatus + 4);
            }
            ...
            if (statusValue) {
                ...
                formData.append('level', 'coo');
                formData.append('amount', <?= $requested->amount; ?>);
                formData.append('project', <?= ($requested->project ? $requested->project : 0); ?>);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success === true) {
                            ...
                            window.location.href = "https://staff.stanforteedge.com/requests/approvals/request/?id=" + id;
                        } else {
                            alert(response.message);
                            window.location.href = "https://staff.stanforteedge.com/requests/approvals/request/?id=" + id;
                        }
