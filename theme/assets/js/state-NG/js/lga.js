$(document).ready(function () {
  // Default profile ID
  var profileId = $("#applicant_id");

  // Get data from API
  $.ajax({
    url: "https://application.carringtonfellows.org/wp-json/jet-cct/applicants",
    data: {
      _ID: profileId,
    },
    dataType: "json",
    success: function (data) {
      var defaultState = data.state || "Select State...";
      var defaultLga = data.lga || "Select LGA...";
      ...
      $.each(states, function (index, state) {
        stateSelect.append(
          $("<option></option>").attr("value", state).text(state)
        );
      });
      stateSelect.val(defaultState);

      // Populate LGA select field
      var lgaSelect = $("#lga");
      var lgas = getLgasForState(defaultState);
      $.each(lgas, function (index, lga) {
        lgaSelect.append($("<option></option>").attr("value", lga).text(lga));
      });
      lgaSelect.val(defaultLga);
    },
  });
  ...
  $("#state").on("change", function () {
    ...
    var lgas = getLgasForState(state);
    $.each(lgas, function (index, lga) {
      lgaSelect.append($("<option></option>").attr("value", lga).text(lga));
    });
    lgaSelect.val("Select LGA...");
  });

  // Helper function to get list of Nigerian states
  ...
  // Helper function to get the list of LGAs for a given state
  function getLgasForState(state) {
    var lgas = [];
    switch (state) {
      case "Abia":
        lgas = [ "Aba North", "Aba South", "Arochukwu", "Bende", "Ikwuano", "Isiala Ngwa North", "Isiala Ngwa South", "Isuikwuato", "Obi Ngwa", "Ohafia", "Osisioma", "Ugwunagbo", "Ukwa East", "Ukwa West", "Umuahia North", "Umuahia South", "Umu-Neochi",
        ];
        break;
      case "Adamawa":
      ...
      case "Ekiti":
        ...
        break;
      case "Rivers":
        lgas = [ "Port Harcourt", "Obio-Akpor", "Okrika", "Ogu–Bolo", "Eleme", "Tai", "Gokana", "Khana", "Oyigbo", "Opobo–Nkoro", "Andoni", "Bonny", "Degema", "Asari-Toru", "Akuku-Toru", "Abua–Odual", "Ahoada West", "Ahoada East", "Ogba–Egbema–Ndoni", "Emohua", "Ikwerre", "Etche", "Omuma",
        ];
        break;
      case "Enugu":
