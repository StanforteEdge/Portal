...
if (isset($_POST['signup'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($user) {
            ...
            $confirmation_key = md5($email . time()); // Generate a unique confirmation key

            update_user_meta($user_id, 'user_activation_key', $confirmation_key); // Store the confirmation key in user meta data

            $subject = 'Confirm your email address';
            $message = 'Please click the following link to confirm your email address: ' . home_url('/confirm-email/?key=' . $confirmation_key);

            wp_mail($user_email, $subject, $message); // Send the confirmation email

            $login_url = home_url();
            $login_url = add_query_arg('signup', 'success', $login_url);
            re_direct($login_url);
            exit;
            ...
            if (is_wp_error($user_id)) {
                ...
                global $wpdb;
                $table_name = $wpdb->prefix . 'jet_cct_applicants';
            ...
            <div class="h-screen xl:h-auto flex py-5 xl:py-0 my-10 xl:my-0">
                <div class="my-auto mx-auto xl:ml-20 bg-white dark:bg-darkmode-600 xl:bg-transparent px-5 sm:px-8 py-8 xl:p-0 rounded-md shadow-md xl:shadow-none w-full sm:w-3/4 lg:w-2/4 xl:w-auto">
                    ...
                    <div class="intro-x mt-8">
                        <form method="post">
                            <input type="text" name="email" id="email" class="intro-x login__input input-form form-control py-3 px-4 block" placeholder="Email " required>
                            <input type="text" name="first_name" id="first_name" class="intro-x  input-form login__input form-control py-3 px-4 block mt-1" placeholder="First Name " required>
                            <input type="text" name="last_name" id="last_name" class="intro-x mt-1 input-form  login__input form-control py-3 px-4 block" placeholder="Last Name " required>
                            <input type="password" name="password" id="password" class="intro-x mt-1  input-form login__input form-control py-3 px-4 block " placeholder="Password" required>
                            <input type="password" name="con_password" id="con_password" class="intro-x mt-1  input-form login__input form-control py-3 px-4 block " placeholder="Confirm Password" required>
                            <div class="intro-x mt-5 xl:mt-8 text-center xl:text-left">
                                <input id="submit_button" type="submit" name="signup" value="Sign Up" class="btn btn-primary py-3 px-4 w-full text-black xl:w-32 xl:mr-3 align-top">
                                <a href="/login" class="btn btn-outline-primary py-3 px-4 w-full text-black xl:w-32 xl:mr-3 mt-2 xl:mt-0 align-top">Login</a>
                                <input type="hidden" name="redirect_to" value="<?php echo esc_url(home_url('/sign-up')); ?>">
