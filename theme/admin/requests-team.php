<?php /* Template Name:  Admin: Requests - Team */
?>
...
$teams = $wpdb->get_results("SELECT * FROM staff_jet_cct_teams a");
?>

<?php if ($staff->team_status == 2):?>
<div class="flex  flex-row justify-between">
    <h2 class="h-full py-5 font-medium flex align-center justify-start text-2xl">Team Requests</h2>
...
<div id="menu-sort" class="intro-y hidden md:flex sm:no-wrap flex-wrap w-full justify-between gap-6 mt-5">
    <div class="flex  flex-wrap sm:w-auto items-center justify-start align-center">
    ...
    jQuery(document).ready(function($) {
        ...
        function checkStatus(status) {
            ...
            if (status == 1) {
                ...
                stat = '<div class="text-primary">Approved</div>';
            } else if (status == 4) {
                stat = '<div class="text-secondary">Un-reimbursed</div>';
            } else {
        ...
        function updateResultDiv(data) {
            ...
            $.each(data.results, function(index, request) {
                ...
                $('#table-list').append('<tr><td class="text-start whitespace-nowrap"><a href="/requests/request/?id=' + request._ID + '" class="underline font-medium decoration-dotted ml-1">' + formatCode(request.code, request._ID) + '</a></td><td class="text-start whitespace-nowrap">' + formatAmount(request.amount) + '</td><td class="text-start whitespace-nowrap">' + formatDate(request.cct_created) + '</td><td><a class="font-medium whitespace-nowrap" href="/admin/applications/applicant/?id=' + request._ID + '">' + request.first_name + ' ' + request.last_name + '</a><div class="hidden text-slate-500 text-xs whitespace-nowrap mt-0.5">' + request.first_name + '</div></td><td class="text-center">' + request.team_name + '</td><td class="w-40">' + formatDate(request.due_date) + '</td><td class="table-report__action w-56">' + checkStatus(request.status) + '</tr>');
            });
        }
