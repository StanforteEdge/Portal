...
8. Verify API contracts before wiring UI filters.
9. Do not assume query param names; confirm controller/service accepted fields first.
10. Never send placeholder filter values (for example `all`, empty strings, or UI labels) as live API filters.
11. If backend expects IDs, send IDs. If backend expects names, send names. Confirm before implementation.
12. Keep list pages resilient to partial API responses by using explicit fallbacks for field names.
13. When introducing new filtering/sorting/pagination, validate with a quick request trace and typecheck before handoff.
