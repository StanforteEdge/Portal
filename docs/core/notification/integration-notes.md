...
- Notifications are sent in-app and/or via email/SMS based on user preferences.
- Reminders are scheduled for time-sensitive actions (e.g., retirements, approvals due, etc.).
...
- In-app notifications are real-time (websocket/polling) and persist in DB.
- Email/SMS sent via external service (SMTP/SMS gateway).
- Reminders are scheduled and sent by background workers at specified times.

## 4. Example Integration Flow
- On request approval:
  1. Workflow engine triggers notification to requester and next approver.
