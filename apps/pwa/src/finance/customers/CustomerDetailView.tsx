...
export function CustomerDetailView({ customerId, contactType, onEdit }: { customerId: string; contactType: "customer" | "vendor"; onEdit: (c: ContactRecord) => void }) {
  const [searchParams, setSearchParams] = useSearchParams();
  const [activeTab, setActiveTab] = useState<"info" | "contacts" | "transactions">("info");

  const { data: contact } = useCachedQuery(
    `finance:contact:${customerId}`,
    () => financeApi.getContact(customerId),
    ...
    <>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: contactData.name || customerId.slice(0, 8) },
        ]}
        title={contactData.company_name || contactData.name || "Customer Details"}
        actions={
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard label="Credit Limit" value={asMoney(contactData.credit_limit)} tone="neutral" />
