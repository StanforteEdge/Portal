...
export default function FinanceVendorsPage() {
  ...
  const [form, setForm] = useState<any>(emptyForm("vendor"));
  ...
  if (contactId) {
    return (
      ...
      >
        <VendorDetailView vendorId={contactId} contactType="vendor" onEdit={openEdit} />
        ...
        action={
          totalContacts > 0 ? (
            <Chip variant="neutral">
              ...
              {Math.min(totalContacts, page * perPage)} of {totalContacts} vendor{totalContacts !== 1 ? "s" : ""}
            </Chip>
          ) : undefined
        }
      >
