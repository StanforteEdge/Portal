    ...
    <main className="relative flex min-h-screen flex-col overflow-hidden bg-surface text-on-surface">
      ...
      <section className="mx-auto hidden w-full max-w-7xl items-center justify-between px-8 py-8 md:flex">
        <div className="flex items-center gap-4">
          <img
            src="/stanforte-logo.png"
            alt="Stanforte Edge"
            className="h-10 w-auto rounded-md bg-white/90 p-1"
          />
          <p className="font-headline text-lg font-bold tracking-tight text-primary">Stanforte Edge</p>
        </div>
      ...
      <section className="relative z-10 mx-auto flex w-full max-w-7xl flex-1 items-center px-6 pb-12 pt-4 md:px-10">
        <div className="hidden w-full items-stretch gap-8 md:grid md:grid-cols-[minmax(0,480px)_minmax(0,1fr)]">
          <div className="rounded-3xl border border-white/60 bg-white/85 p-10 shadow-card backdrop-blur-xl">
            <h1 className="font-headline text-2xl font-bold text-primary">Staff Portal Access</h1>
            <p className="mt-2 text-sm text-on-surface-variant">
              Secure login for authorized Stanforte Edge personnel.
            </p>

            {error ? (
              <div role="alert" className="mt-6 rounded-2xl border border-danger/20 bg-danger/10 px-4 py-3 text-sm text-danger">
                {error}
              </div>
            ) : null}
            ...
            <form className="mt-8 space-y-6" onSubmit={handleSubmit} noValidate>
              <label className="block">
                ...
                <div className="relative">
                  <Icon name="mail" className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                  <input
              ...
              <label className="block">
                ...
                <div className="relative">
                  <Icon name="lock" className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                  <input
                    className="input-base pl-10"
          ...
          <aside className="rounded-3xl border border-white/60 bg-gradient-to-br from-primary-fixed/40 to-secondary-fixed/30 p-8 shadow-soft">
            <div className="rounded-2xl bg-white/80 p-6 backdrop-blur">
              <h2 className="font-headline text-2xl font-bold text-primary">One Portal, Daily Workflows</h2>
              <p className="mt-2 text-sm leading-6 text-on-surface-variant">
                Handle staff requests, attendance, and approvals in one secure workspace built for multi-organization teams.
              </p>

              <div className="mt-6 grid gap-3">
                {[
                  { icon: "assignment", title: "Requests & Approvals", text: "Submit, review, and track request decisions end-to-end." },
                  { icon: "pending_actions", title: "Attendance Ops", text: "Clock-in, corrections, and exceptions with clear audit trails." },
                  { icon: "security", title: "Security by Default", text: "Session protection, role controls, and monitored access." },
                ].map((item) => (
                  <article key={item.title} className="rounded-2xl bg-white px-4 py-4">
        ...
        <div className="mx-auto w-full max-w-sm md:hidden">
          <div className="mb-10 text-center">
            <img src="/stanforte-logo.png" alt="Stanforte Edge" className="mx-auto h-12 w-auto rounded-md bg-white/85 p-1" />
            <h1 className="mt-5 font-headline text-3xl font-extrabold tracking-tight text-primary">Portal</h1>
            <p className="mt-1 text-sm text-secondary">Stanforte Edge Staff Ecosystem</p>
          </div>

          <div className="rounded-3xl bg-white p-7 shadow-soft">
            <h2 className="font-headline text-xl font-semibold text-on-surface">Welcome back</h2>
            <p className="mt-1 text-sm text-on-surface-variant">Please enter your credentials to continue</p>
