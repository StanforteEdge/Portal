...
import { listRequestTypes, type RequestTypeOption } from "./requests-api";
import {
...
const familyCards: FamilyCard[] = [
  {
    ...
    description: "Expenses, reimbursements, procurement, and other money-related requests.",
  },
  {
    family: "leave",
    icon: "event_available",
    description: "Annual leave, sick leave, and other time-off requests with approval routing.",
  },
  {
    family: "other",
    icon: "assignment",
    description: "Other request workflows as the platform expands across teams and organizations.",
  },
];
  ...
  return (
    <div
      className={[
        ...
        active ? "border-brand-900 ring-2 ring-brand-900/10" : "border-slate-200",
      ].join(" ")}
    >
      ...
      <h2 className="mt-5 text-lg font-semibold text-slate-950">{requestFamilyLabel(family)} Requests</h2>
...
export function RequestTypePage() {
  ...
  const activeFamily: RequestFamily | null =
    familyParam === "financial" || familyParam === "leave" || familyParam === "other"
      ? familyParam
      : null;
    ...
    >
      <div className="hidden lg:block">
        ...
        <div className="grid gap-6 lg:grid-cols-12">
          <div className="space-y-6 lg:col-span-8">
            ...
            <SectionCard
              title={activeFamily ? `${requestFamilyLabel(activeFamily)} request types` : "Choose a family first"}
              description={
                activeFamily
            ...
            >
              {!activeFamily ? (
                ...
                <div className="rounded-2xl bg-slate-50 px-4 py-6 text-sm text-slate-500">Loading request types...</div>
              ) : error ? (
                  ...
                  {selectedTypes.map((type) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-lg font-semibold text-slate-950">{type.name}</p>
                          <p className="mt-2 text-sm leading-6 text-slate-500">
                            {type.description || "Open this workflow with the right fields and approval path."}
                          </p>
                        </div>
                        <Chip variant="neutral">{requestFamilyLabel(requestFamilyFromType(type))}</Chip>
                      </div>
          ...
          <div className="space-y-6 lg:col-span-4">
            ...
            <SectionCard title="Need help choosing?">
              <p className="text-sm leading-6 text-slate-600">
                If you are not sure which request subtype to use, start with the family that best matches
                the outcome you need: funds, leave, or another internal workflow.
              </p>
      ...
      <div className="space-y-4 lg:hidden">
        <div className="pt-1">
          ...
          <h1 className="page-title mt-2 text-[clamp(1.7rem,7vw,2.2rem)]">Create a new request</h1>
          <p className="mt-3 max-w-xl text-sm leading-6 text-slate-500">
            Choose a request family first, then select the subtype to open the right form.
          </p>
        </div>
            ...
            <div className="mt-4 grid gap-3">
              {selectedTypes.map((type) => (
                ...
                >
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-500">
                    {type.description || "Open the right request form for this subtype."}
                  </p>
                </Link>
              ))}
            </div>
