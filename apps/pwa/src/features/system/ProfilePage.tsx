...
import { Button, Chip, SectionCard, StatCard, TextAreaField, TextField, humanize, roleLabel, sortRoles, userDisplayName } from "@stanforte/shared";
...
export default function ProfilePage() {
  ...
  async function saveProfile() {
    try {
      ...
      setNotice(updateError instanceof Error ? updateError.message : "Unable to update profile.");
    } finally {
      setSaving(false);
    }
  }
        ...
        <div className="space-y-6 lg:col-span-8">
          ...
          <SectionCard title="Overview">
              ...
              <div>
                ...
                <div className="mt-3 space-y-2">
                  <div className="flex flex-wrap items-center gap-2 text-sm text-slate-600">
                    ...
                    {extraRoles.length ? (
                      <Chip variant="neutral">+{extraRoles.length} other{extraRoles.length > 1 ? "s" : ""}</Chip>
                    ) : null}
                  </div>
                  <div className="flex flex-wrap items-center gap-2 text-sm text-slate-600">
                    <span className="font-semibold text-slate-900">Title:</span>
                    <span>{profile?.employee_profile?.job_title?.trim() || "-"}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <StatCard label="Employee Code" value={profile?.employee_profile?.employee_code || "-"} tone="neutral" />
              <StatCard label="Onboarding" value={humanize(profile?.onboarding_progress?.status)} tone="warning" />
              <StatCard label="Employment" value={humanize(profile?.employee_profile?.employment_status)} tone="neutral" />
          ...
          <SectionCard title="Edit Profile">
            <div className="grid gap-4 md:grid-cols-2">
              <TextField label="First Name" value={form.first_name} onChange={(event) => setForm((prev) => ({ ...prev, first_name: event.target.value }))} />
              <TextField label="Last Name" value={form.last_name} onChange={(event) => setForm((prev) => ({ ...prev, last_name: event.target.value }))} />
            ...
            <div className="mt-4">
              <TextAreaField label="Address" value={form.address} onChange={(event) => setForm((prev) => ({ ...prev, address: event.target.value }))} />
            </div>
            <Button className="mt-4" onClick={() => void saveProfile()} disabled={saving}>
          ...
          <SectionCard title="Organizations">
            <div className="space-y-2">
              {organizations.length ? organizations.map((org) => (
                <div key={org.id} className="rounded-[18px] border border-slate-100 bg-slate-50 px-4 py-3">
                  <p className="text-sm font-semibold text-slate-800">{org.name}</p>
                  <p className="mt-1 text-xs text-slate-500">{org.code}{org.is_primary ? " • Primary" : ""}</p>
                </div>
